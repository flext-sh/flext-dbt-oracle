...
class TestsFlextDbtOracleConfig:
    ...
    def test_materialization_is_a_free_scalar_string(self) -> None:
        ...
        oracle = FlextDbtOracleSettings(
            FlextDbtOracleSettings._DbtOracle(materialization="custom")
        ).DbtOracle
    ...
    def test_pool_bounds_round_trip_through_dboracle(self) -> None:
        ...
        settings = FlextDbOracleSettings(DbOracle=FlextDbOracleSettings.DbOracleSettings(pool_min=5, pool_max=5))
    ...
    def test_numeric_fields_retain_supplied_values(self) -> None:
        ...
        settings = FlextDbOracleSettings(
            DbOracle=FlextDbOracleSettings.DbOracleSettings(port=1521, pool_min=1, pool_max=50, timeout=60)
        )
    ...
    def test_every_valid_materialization_is_accepted(
        ...
        oracle = FlextDbtOracleSettings(
            FlextDbtOracleSettings._DbtOracle(materialization=materialization)
        ).DbtOracle
    ...
    def test_sid_and_service_name_coexist_on_dboracle(self) -> None:
        ...
        settings = FlextDbOracleSettings(
            DbOracle=FlextDbOracleSettings.DbOracleSettings(sid="XE", service_name="XEPDB1")
        )
